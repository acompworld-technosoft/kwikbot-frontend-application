...
import { sendSetPasswordotp, verifyotp } from "../../../Services/Admin/forgotPasswordapi";
...
const SetPassword = () => {
 ...
 useEffect(() => {
  if (emailParam) {
    setResetInfo((prevInfo) => ({ ...prevInfo, email: emailParam }));
  }
}, [emailParam]);
  ...
  useEffect(() => {
    ...
    if (email) {
      setResetInfo((prevInfo) => ({ ...prevInfo, email }));
    }
  }, []);
  ...
  const handleSendOtp = async () => {
    ...
    if (Object.keys(newError).length === 0) {
      try {
        ...
        const sendOtpResponse = await sendSetPasswordotp({ email: resetInfo.email });
